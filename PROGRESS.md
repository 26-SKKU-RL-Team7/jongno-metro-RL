...
- Added graph-based RL policy module `src/gnn_lite_policy.py` with station graph encoding, masked action selection, and manual policy-gradient backprop helpers.
- Added `src/train_jongno_gnn_lite.py` for GNN-lite REINFORCE training and model export.
- Added `src/compare_jongno_policies.py` to train/evaluate linear baseline vs GNN-lite and report reward/score deltas.
- Extended `src/visualize_jongno_policy.py` to support both linear and GNN model playback (`--model-type`).
- Added `test/test_gnn_lite_policy.py` coverage for GNN-lite forward/sampling/backward/update paths.
- Updated `README.md` with GNN-lite train/visualize and policy comparison commands.
- Added lightweight RL policy utilities in `src/jongno_policy.py` (feature vector, action mask, stochastic/greedy action selection).
- Added `src/train_jongno_policy.py` for NumPy REINFORCE training and model export to `.npz`.
- Added `src/visualize_jongno_policy.py` to replay trained policy with live simulator rendering.
- Added `test/test_jongno_policy.py` for policy feature/mask behavior coverage.
- Updated `README.md` with train/visualize commands for the new lightweight RL workflow.
- Added Korean documentation `PROJECT_GUIDE_KR.md` with project overview, MDP/task/reward modification guide, and GitHub upload checklist.
- Linked `PROJECT_GUIDE_KR.md` from `README.md`.
- Added `INSTRUCTION_TASK_RUNNER.md` with editable instructions for adding and running new tasks/policies.
- Linked task-runner instruction doc from `README.md`.
- Added task-based experiment architecture with `src/task_catalog.py` and `src/task_runner.py` so different problem settings can be selected by task id.
- Added built-in task presets (`jongno_dispatch`, `jongno_peak_stress`) and policy plug-in loading via `module:function`.
- Added `test/test_task_runner.py` and updated `README.md` with task-based run commands.
- Updated `Mediator` passenger routing to support explicit `destination_station_name` (exact station destination).
- Refactored `src/jongno_env.py` to use `jongno_config.json` demand (`spawn_rates`, `dest_probs`) for demand-driven passenger spawning and destination sampling.
- Redesigned `JongnoMetroEnv` reward to use `Δscore` + waiting penalty + invalid-action penalties (instead of cumulative score).
- Fixed `preprocess_jongno.py` to deduplicate line station sequences while preserving order.
- Added `__future__` annotations and restored `ui.viewport.get_viewport_transform` compatibility for Python 3.9 unit tests.
