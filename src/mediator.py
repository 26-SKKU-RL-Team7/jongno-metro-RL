...
class Mediator:
    def __init__(self, is_rl_mode: bool = False) -> None:
        pygame.font.init()
        self.is_rl_mode = is_rl_mode
    ...
    def apply_action(self, action: Dict) -> bool:
        ...
        if action_type == "add_train":
            path_id = action.get("path_id")
            path = self.get_path_by_id(path_id)
            if path and len(self.metros) < self.num_metros:
                metro = Metro()
        ...
        if action_type == "remove_train":
            path_id = action.get("path_id")
            path = self.get_path_by_id(path_id)
            if path:
                for metro in path.metros:
                    if not getattr(metro, 'is_retiring', False):
                        metro.is_retiring = True
                        return True
            return False
    ...
    def get_unloading_candidates_for_metro(
        ...
        for passenger in metro.passengers:
            if self.is_passenger_destination_station(passenger, station):
    ...
    def move_passengers(self, dt_ms: int) -> None:
        ...
        for metro in self.metros:
            if metro.current_station:
                ...
                if getattr(metro, 'is_retiring', False):
                    boarding_candidates = []
                else:
                    boarding_candidates = self.get_boarding_candidates_for_metro(
                        metro,
        ...
        for metro in list(self.metros):
            if getattr(metro, 'is_retiring', False):
                if len(metro.passengers) == 0 and metro.speed == 0:
                    path = self.get_path_by_id(metro.path_id)
                    if path and metro in path.metros:
    ...
    def is_passenger_destination_station(
        self, passenger: Passenger, station: Station
    ) -> bool:
        """
        RL extension: if a passenger carries an explicit destination station name,
        treat only that station as their destination.
        Otherwise, fall back to the original "station.shape.type == destination_shape.type".
        """
        destination_station_name = getattr(passenger, "destination_station_name", None)
        if destination_station_name is not None:
            return getattr(station, "name", None) == destination_station_name
        return station.shape.type == passenger.destination_shape.type

    def get_destination_stations_for_passenger(
        self, passenger: Passenger
    ) -> List[Station]:
        destination_station_name = getattr(passenger, "destination_station_name", None)
        if destination_station_name is not None:
            # Stable order (no shuffle): demand-based routing should be deterministic.
            return [
                station
                for station in self.stations
                if getattr(station, "name", None) == destination_station_name
            ]
        return self.get_stations_for_shape_type(passenger.destination_shape.type)
