...
from jongno_line_policy import sample_line_action
...
def train(
    *,
    ...
    feature_dim = int(getattr(env, "policy_feature_dim", obs.size + 1))
    ...
    for episode_idx in range(episodes):
        ...
        for _ in range(max_steps):
            action, probs, features = sample_line_action(weights, env, obs, rng)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train masked softmax policy for JongnoLineSelectionEnv (stage 1)"
    )
    parser.add_argument("--task", default="jongno_line_select", help="Task ID")
    ...
    parser.add_argument(
        "--output",
        default="artifacts/jongno_line_policy.npz",
        help="Output .npz path for trained weights",
    )
    parser.add_argument(
        "--env-overrides",
        default=None,
        help='JSON dict, e.g. \'{"line_eval_rollout_steps": 80}\'',
    )
    return parser
